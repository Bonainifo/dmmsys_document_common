...
class DocumentFolder(models.Model):
    ...
    def default_get(self, fields):
        ...
        if 'parent_id' in fields and self._context.get('folder_id') and not res.get('parent_id'):
    ...
    parent_id = fields.Many2one('documents.folder',
    ...
    children_folder_ids = fields.One2many('documents.folder', 'parent_id', string="Sub workspaces")
    document_ids = fields.One2many('document.file', 'folder_id', string="Documents")
    ...
    category_ids = fields.One2many('documents.category', 'folder_id', copy=True,
    ...
    type = fields.Selection(selection=[('common','普通工作区')],default="common")

    def _compute_deletion_delay(self):
        self.deletion_delay = self.env['document.file'].get_deletion_delay()
    ...
    def _compute_has_write_access(self):
        ...
        has_write_access = self.env.user.has_group('dmmsys_document_common.group_documents_manager')
        if has_write_access:
    ...
    def _get_old_id_to_new_id_maps(self, copied_folder):
        ...
        mapping original categorys and tags to the copied ones, that will be used
        ...
        """
        self.ensure_one()
        ancestors_category_map = self.env.context.get('ancestors_category_map') or {}
        ...
        copied_folder_category_map, copied_folder_tag_map = [get_old_id_to_new_id_map(self.id, copied_folder.id, table) for table in ('documents_category', 'documents_tag')]
        return {**ancestors_category_map, **copied_folder_category_map}, {**ancestors_tag_map, **copied_folder_tag_map}

    def _copy_workflow_rules_and_actions(self, copied_folder, old_category_id_to_new_category_id, old_tag_id_to_new_tag_id):
        self.ensure_one()
        ...
        for workflow_action in old_workflow_actions:
            category_id, tag_id = False, False
            if workflow_action.category_id:
                # If the category/tag comes from an ancestor (including the folder itself)
                # that is being copied, update its value to the copied category/tag.
                # If the ancestor is not being copied, remove the value.
                if str(workflow_action.category_id.folder_id.id) in self.parent_path[:-1].split('/'):
                    category_id = old_category_id_to_new_category_id.get(workflow_action.category_id.id, False)
                    tag_id = workflow_action.tag_id.id and old_tag_id_to_new_tag_id.get(workflow_action.tag_id.id, False)
                # If the category/tag comes from an unrelated folder, keep the values
                else:
                    category_id = workflow_action.category_id.id
    ...
    def copy(self, default=None):
        ...
        old_category_id_to_new_category_id, old_tag_id_to_new_tag_id = self._get_old_id_to_new_id_maps(folder)
        self._copy_workflow_rules_and_actions(folder, old_category_id_to_new_category_id, old_tag_id_to_new_tag_id)

        for child in self.children_folder_ids:
            child.with_context({
                'ancestors_category_map': old_category_id_to_new_category_id,
                'ancestors_tag_map': old_tag_id_to_new_tag_id,
            }).copy({'parent_id': folder.id})
    ...
    def _compute_document_count(self):
        read_group_var = self.env['document.file']._read_group(
            [('folder_id', 'in', self.ids)],
            groupby=['folder_id'],
            aggregates=['__count'])
    ...
    def move_folder_to(self, parent_id=False, before_folder_id=False):
        ...
        :param parent_id: id of the new parent folder
        :param before_folder_id: id of the folder before which to move
        """
        self.ensure_one()
        values = {'parent_id': parent_id}
        parent_folder = self.browse(parent_id)
        if parent_folder and self.parent_id != parent_folder:
        ...
        # its parent
        if before_folder and before_folder.parent_id.id == parent_id:
            values['sequence'] = self.sudo()._resequence_to_insert_before(before_folder)
        else:
            values['sequence'] = self._get_max_sequence_inside_parent(parent_id) + 1
    ...
    def _resequence_to_insert_before(self, before_folder):
        ...
        folders_to_resequence = self.search([
            ("parent_id", "=", before_folder.parent_id.id),
    ...
    def action_unarchive(self):
        ...
        self.filtered(lambda folder: not folder.parent_id[self._active_name]).parent_id = False
    ...
    def unlink(self):
        ...
        """
        removable_parent_folders = self.parent_id.with_context({'active_test': False}).filtered(
