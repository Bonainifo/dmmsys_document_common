...
from .documents_category import N_FACET_COLORS
import logging
...
class Document(models.Model):
    ...
    type = fields.Selection(related="folder_id.type",store=True)
    ...
    def get_deletion_delay(self):
        return int(self.env['ir.config_parameter'].sudo().get_param('dmmsys_document_common.deletion_delay', '30'))
    ...
    def access_content(self):
        ...
        if self.url:
            ...
            action['url'] = '/dmmsys_document_common/content/%s' % self.id
        return action
    ...
    def toggle_lock(self):
        ...
        Members of the group dmmsys_document_common.group_document_manager and the superuser can unlock the file regardless.
        """
        self.ensure_one()
        if self.lock_uid:
            if self.env.user == self.lock_uid or self.env.is_admin() or self.user_has_groups(
                    'dmmsys_document_common.group_document_manager'):
                self.lock_uid = False
        else:
    ...
    def _compute_is_locked(self):
        for record in self:
            record.is_locked = record.lock_uid and not (
                    ...
                    self.user_has_groups('dmmsys_document_common.group_document_manager'))
    ...
    @api.model
    def verify_domain_query(self, domain):
        """
        验证 domain 并执行查询
        """
        valid_domain = []
        ...
        if invalid_fields:
            raise ValueError(f"Invalid fields in domain: {invalid_fields}")

        # 使用有效的 domain 进行搜索
        documents = self.search(valid_domain)
        return documents
    ...
    def _get_processed_tags(self, domain, folder_id):
        """
        sets a group color to the tags based on the order of the categorys (group_id)
        ...
        categorys = list(OrderedDict.fromkeys([tag['group_id'] for tag in tags]))
        for tag in tags:
            color_index = (categorys.index(tag['group_id']) % N_FACET_COLORS) + 1
    ...
    def search_panel_select_multi_range(self, field_name, **kwargs):
        ...
        # 清理 domain 中的无效字段
        search_domain = self._clean_domain(search_domain)
        category_domain = self._clean_domain(category_domain)
        filter_domain = self._clean_domain(filter_domain)
        ...
        return super(DocumentsDocument, self).search_panel_select_multi_range(field_name, **kwargs)
    ...
    def _clean_domain(self, domain):
        ...
        if invalid_fields:
            _logger.debug("Invalid fields removed from domain: %s", invalid_fields)
