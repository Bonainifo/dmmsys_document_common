...
class Document(models.Model):
    ...
    def _get_permission_without_token(self):
        ...
        if not self.env.user.share and user_permission != "edit" and self.access_internal != 'none':
            if not self.company_id or self.company_id in self.env.companies:
                user_permission = (
                    'edit' if self.env.user.has_group('dmmsys_document_common.group_documents_manager')
    ...
    def _search_user_permission(self, operator, value):
        ...
        if self.env.user.has_group('dmmsys_document_common.group_documents_manager'):
            if searched_roles == ['view']:
                direct_domain = expression.AND([
                    direct_domain,
    ...
    def _get_folder_embedded_actions(self, folder_ids):
        ...
        embedded_actions = self.env['ir.embedded.actions'].sudo().search(
            domain=[
                ('parent_action_id', '=', self.env.ref("dmmsys_document_common.document_action").id),
                ('action_id', '!=', False),
    ...
    def action_change_owner(self, new_user_id):
        if not self.env.user._is_admin() and not self.env.user.has_group('dmmsys_document_common.group_documents_manager'):
    ...
    def action_folder_embed_action(self, folder_id, action_id, groups_ids=None):
        ...
        """
        if not self.env.user.has_group('dmmsys_document_common.group_documents_user'):
    ...
    def toggle_lock(self):
        ...
        Members of the group dmmsys_document_common.group_documents_manager and the superuser can unlock the file regardless.
        """
        self.ensure_one()
        if self.lock_uid:
            if self.env.user == self.lock_uid or self.env.is_admin() or self.env.user.has_group('dmmsys_document_common.group_documents_manager'):
                self.lock_uid = False
        else:
    ...
    def _compute_is_locked(self):
        for record in self:
            record.is_locked = record.lock_uid and not (
                    ...
                    self.env.user.has_group('dmmsys_document_common.group_documents_manager'))
    ...
    def create(self, vals_list):
        ...
        is_manager = self.env.is_admin() or self.env.user.has_group('dmmsys_document_common.group_documents_manager')
        if not is_manager:
    ...
    def can_upload_traceback(self):
        return self.env.user._is_internal and \
            bool(self.env.ref('dmmsys_document_common.document_support_folder', raise_if_not_found=False))
    ...
    def _get_access_action(self, access_uid=None, force_website=False):
        self.ensure_one()
        if access_uid and not force_website and self.active and self.env.user.has_group("dmmsys_document_common.group_documents_user"):
            url_params = url_encode({
                'preview_id': self.id,
                'view_id': self.env.ref("dmmsys_document_common.document_view_kanban").id,
                'menu_id': self.env.ref("dmmsys_document_common.menu_root").id,
                'folder_id': self.folder_id.id,
            })
