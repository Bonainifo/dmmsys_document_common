    ...
    def _documents_render_public_view(self, document_sudo):
        ...
        if target_sudo or not document_sudo:
            return request.render(
                'dmmsys_document_common.not_available', {'document': document_sudo}, status=404)
        ...
        if document_sudo.type == 'binary' and document_sudo.attachment_id:
            return request.render('dmmsys_document_common.share_file', {'document': document_sudo})
        if document_sudo.type == 'binary':
            return request.render('dmmsys_document_common.document_request_page', {'document': document_sudo})
        if document_sudo.type == 'folder':
            sub_documents_sudo = ShareRoute._get_folder_children(document_sudo)
            return request.render('dmmsys_document_common.public_folder_page', {
