...
class ShareRoute(http.Controller):
    ...
    @http.route('/dmmsys_document_common/upload_attachment', type='http', methods=['POST'], auth="user")
    def upload_document(self, folder_id, ufile, tag_ids, document_id=False, partner_id=False, owner_id=False, res_id=False, res_model=False):
        ...
        if document_id:
            ...
            try:
                ...
                request.env['documents.mixin']._upload_create_hook(document)
            ...
            #执行上传后的处理
            request.env['documents.mixin']._upload_create_hook(documents)
            result['ids'] = documents.ids

        return json.dumps(result)

    @http.route('/dmmsys_document_common/pdf_split', type='http', methods=['POST'], auth="user")
    ...
    @http.route(['/dmmsys_document_common/content/<int:id>'], type='http', auth='user')
    def documents_content(self, id):
        return self._get_file_response(id)

    @http.route(['/dmmsys_document_common/pdf_content/<int:document_id>'], type='http', auth='user')
    ...
    @http.route(['/dmmsys_document_common/image/<int:res_id>',
                 '/dmmsys_document_common/image/<int:res_id>/<int:width>x<int:height>',
        ...
        try:
            ...
            if available_documents is False:
                if share._check_token(token):
                    ...
                    return request.render('dmmsys_document_common.not_available', options)
                else:
                    return request.not_found()
            ...
            if len(shareable_documents) == 1 and shareable_documents.type == 'empty':
                return request.render("dmmsys_document_common.document_request_page", options)
